"""The TPTP language (TFF, typed first-order form), read into SymPy formulas.

:func:`problem` reads a TPTP problem of the arithmetic fragment into a
:class:`TPTPProblem`: one closed formula — the axioms imply the conjecture,
with every free variable universally quantified — over ``$int`` or
``$real``, and the ``Status`` its header records (:data:`STATUS` says what
``resolve`` must return for it).
...
>>> from sympy_extras_benchmarks.parsers.tptp import problem
>>> text = '''
... % Status   : Theorem
... tff(easy,conjecture,
...     ! [X: $int,Y: $int] :
...
__all__ = ['TPTPProblem', 'TPTPError', 'TYPES', 'REFUSED_TYPES', 'STATUS', 'problem']
...
#: a typed binder, and a numeral which is not an integer
_BINDER = re.compile(r'[!?]\s*\[')
_NON_INTERO = re.compile(r'\d+\.\d+|\d+/\d+')
...
def problem(text: str, name: str = '') -> Optional[TPTPProblem]:
    ...
    if not domains:
        # a problem without variables declares no type: the numerals say it
        # (in TPTP ``2`` is ``$int``, ``2.0`` ``$real``, ``2/3`` ``$rat``).
        # With nothing to quantify over, the choice of domain cannot change
        # the answer, and the distinction between Q and R -- decisive for the
        # quantified ``$rat`` problems -- has no purchase here.
        if _BINDER.search(body):
            return None                     # a variable of undeclared type
        domains = {'reals' if _NON_INTERO.search(body) else 'integers'}
    if len(domains) != 1:
        return None
    ...
    for _, role, source in annotations:
        ...
        try:
            ...
            if parser.peek() is not None:
                return None
        except (TPTPError, TypeError, ValueError, RecursionError):
            return None
        if role == 'conjecture':
